...
docker run -p 8000:8000 --name workato_splunk -d -e "SPLUNK_START_ARGS=--accept-license" -e "SPLUNK_PASSWORD=%SPLUNK_PASSWORD%" -e "WORKATO_INTEGRATION_TEST=1" -v "%DEPOY_PATH%/../:/opt/splunk/etc/apps/workato_addon_for_splunk:rw" -v "%DEPOY_PATH%/test_app:/opt/splunk/etc/apps/test_app:rw" --network %NETWORK_NAME% splunk/splunk:7.2.0
